                    ...
                    <div class="card-header">
                        <div class="row g-4">
                            <div class="col-sm-auto">
                                <h4 class="card-title mt-2">DATA GOLONGAN & UKT</h4>
                            </div>
                        </div>
                    </div><!-- end card header -->
                    <div class="card-body">
                                ...
                                <div>
                                    <button type="button" class="btn btn-primary add-btn" data-bs-toggle="modal"
                                        id="create-btn" data-bs-target="#addModal"><i
                                            class="ri-add-line align-bottom me-1"></i>Tambah Golongan</button>
                                </div>
                            </div>
                            <div class="col-sm">
                        ...
                        <div class="table-responsive table-card mt-3 mb-1">
                            <table class="table align-middle table-nowrap" id="customerTable">
                                <thead class="table-light">
                                    <tr>
                                        <th>No</th>
                                        <th class="sort" data-sort="customer_name">GOLONGAN</th>
                                        <th class="sort" data-sort="date">KRITERIA GOLONGAN</th>
                                ...
                                <tbody class="list form-check-all">
                                    @foreach ($golongan as $i => $row)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td class="customer_name">{{ $row->nama_golongan }}</td>
                                            <td class="date">{{ $row->kriteria }}</td>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2">
                                                    ...
                                                    <div class="remove">
                                                        <a href="{{ route('golongan-ukt.edit', $row->id) }}" type="button"
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('golongan-ukt.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="email-field" class="form-label">NAMA GOLONGAN</label>
                            <input type="text" name="nama_golongan" id="date-field" class="form-control" required />
                        </div>
                        <div class="mb-3">
                            <label for="email-field" class="form-label">KRITERIA</label>
                            <textarea type="text" name="kriteria" id="date-field" class="form-control" rows="5" style="resize: none"
                                required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <div class="hstack gap-2 justify-content-end">
                            ...
                            <button type="submit" class="btn btn-primary" id="add-btn">Add Golongan</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="modal fade" id="editModal{{ $row->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('golongan-ukt.update', $row->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3" id="modal-id" style="display: none;">
                            ...
                            <div class="mb-3">
                                <label for="email-field" class="form-label">NAMA GOLONGAN</label>
                                <input type="text" name="nama_golongan" id="date-field" class="form-control"
                                    value="{{ $row->nama_golongan }}" required />
                            </div>

                            <div class="mb-3">
                                <label for="phone-field" class="form-label">KRITERIA</label>
                                <input type="text" name="kriteria" id="email-field" class="form-control"
                                    value="{{ $row->kriteria }}" required />
                            </div>
                        </div>
                        <div class="modal-footer">
                            <div class="hstack gap-2 justify-content-end">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Tutup</button>
                                <button type="submit" class="btn btn-warning" id="add-btn">Edit Golongan</button>
                            </div>
                        </div>
                    </form>
                </div>
        ...
        <div class="modal fade zoomIn" id="deleteRecordModal{{ $row->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('golongan-ukt.destroy', $row->id) }}" method="POST"
